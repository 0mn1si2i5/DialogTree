...
func AiRouter(rg *gin.RouterGroup) {
	...
	sessionGroup := rg.Group("/sessions")
	{
		...
		sessionGroup.PUT("/:sessionId", sessionApi.UpdateSession)       // 更新会话信息
	...
	dialogGroup := rg.Group("/dialog")
	{
		...
		dialogGroup.PUT("/conversations/comment", dialogApi.UpdateConversationComment)            // 更新评论
		dialogGroup.DELETE("/conversations/:conversationId", dialogApi.DeleteConversationComment) // 更新评论
	}
